...
namespace ValidateCert
{
    ...
    public class CertificateValidator
    {
        ...
        /// <summary>
        /// Chuỗi chứng thư số (user certificate - certification authority (CA) - Root CA)
        /// </summary>
        ...
        {
            string tmp =  Encoding.ASCII.GetString(WinCrypt32.ConvertHexToByte(crlInfo.Issuer));
        ...
        public CRL_Info GetCRlFromLink(string crlLink)
        {
            ...
            {
                //Download crl từ internet
                WebClient wc = new WebClient();
                rgRawCrl = wc.DownloadData(crlLink);
            }
            else
                //Đọc file crl ở local
    ...
    public class CRL_Info
    {
        ...
        internal CRL_Info(WinCrypt32.CRL_INFO stCrlInfo)
        {
            ...
            _issuer = WinCrypt32.ConvertBLOBToSerial(stCrlInfo.Issuer);
            _thisUpdate = WinCrypt32.ConvertFILETIMEToDate(stCrlInfo.ThisUpdate);
            _nextUpdate = WinCrypt32.ConvertFILETIMEToDate(stCrlInfo.NextUpdate);
            ...
            for (int i = 0; i < stCrlInfo.cCRLEntry; i++)
            {
                ...
                //Get info
                _crlEntries[i].SerialNumber = WinCrypt32.ConvertBLOBToSerial(stCrlEntry.SerialNumber);
                _crlEntries[i].RevocationDate = WinCrypt32.ConvertFILETIMEToDate(stCrlEntry.RevocationDate);
