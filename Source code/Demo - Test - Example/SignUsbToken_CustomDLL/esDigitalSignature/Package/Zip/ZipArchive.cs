...
//-----------------------------------------------------------------------------
//
// <copyright file="ZipArchive.cs" company="Microsoft">
    ...
    internal sealed class ZipArchive : IDisposable
    {
        ...
        internal static ZipArchive OpenOnFile(string path, FileMode mode, FileAccess access, FileShare share, bool streaming)
        {
            if (mode == FileMode.OpenOrCreate || mode == FileMode.Open)
            {
                ...
                {
                    throw new FileFormatException("ZipZeroSizeFileIsNotValidArchive");
                }
            }
            ...
            if (stream.CanSeek)
            {
                ...
                switch (mode)
                {
                    ...
                    case FileMode.CreateNew:
                        if (!emptyStream)
                        {
                            throw new IOException("CreateNewOnNonEmptyStream");
                        }
                        break;
        ...
        internal ZipFileInfo AddFile(string zipFileName, CompressionMethodEnum compressionMethod, DeflateOptionEnum deflateOption)
        {
            ...
            {
                throw new InvalidOperationException("CanNotWriteInReadOnlyMode");
            }
            ...
            {
                throw new System.InvalidOperationException("AttemptedToCreateDuplicateFileName");            
            }
        ...
        internal ZipFileInfo GetFile(string zipFileName)
        {
            ...
            {
                throw new InvalidOperationException("CanNotReadInWriteOnlyMode");
            }
            ...
            else
            {
                ...
                if (!FileExists(zipFileName))
                {
                    throw new InvalidOperationException("FileDoesNotExists");                
                }
        ...
        static internal void VerifyVersionNeededToExtract(UInt16 version)
        {
            ...
            switch (version)
            {
                ...
                default:
                    throw new NotSupportedException("NotSupportedVersionNeededToExtract"); 
            }
        }
...
//  might eventually get public exposure we would like to keep this code around
        ...
        static private void ValidateModeAccessShareStreaming(Stream stream, FileMode mode, FileAccess access, FileShare share, bool streaming)
        {
            ...
            throw new ArgumentException("UnsupportedCombinationOfModeAccessShareStreaming");
        }
        ...
        static private void ValidateModeAccessStreamStreamingCombinations(Stream stream, FileAccess access, bool streaming)
        {
            ...
            {
                throw new ArgumentException("CanNotReadDataFromStreamWhichDoesNotSupportReading");
            }
            ...
            {
                throw new ArgumentException("CanNotWriteDataToStreamWhichDoesNotSupportWriting");
            }            

            // if user works in non-streaming mode we need to Seek on underlying stream 
            if (! streaming && !stream.CanSeek)
            {
                throw new ArgumentException("CanNotOperateOnStreamWhichDoesNotSupportSeeking");
            }            
        }
        ...
        static private void ValidateModeAccessShareSupportedEnums(FileMode mode, FileShare share)
        {
            ...
            {
                throw new NotSupportedException("TruncateAppendModesNotSupported");
            }
            else if (share != FileShare.Read && share != FileShare.None)
            {
                ...
                throw new NotSupportedException ("OnlyFileShareReadAndFileShareNoneSupported");   
            }
        }
        ...
        private void CheckDisposed()
        {
            if (_disposedFlag)
            {
                throw new ObjectDisposedException(null, "ZipArchiveDisposed");
            }
        }
