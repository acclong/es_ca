//-----------------------------------------------------------------------------
//
// <copyright file="CompressStream.cs" company="Microsoft">
...
namespace esDigitalSignature.Package
{
    ...
    internal class CompressStream : Stream
    {
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            if (!CanSeek)
                throw new NotSupportedException("SeekNotSupported");
            ...
            checked
            {
                ...
                if (newPos < 0)
                    throw new ArgumentException("SeekNegative");
        ...
        public override void SetLength(long newLength)
        {
            ...
            if (!CanSeek)
                throw new NotSupportedException("SetLengthNotSupported");
...
//               if (!CanSeek)
//                    throw new NotSupportedException("LengthNotSupported));
            ...
            if (_cachedLength >= 0)
            {
                if (!_lengthVerified)
                {
                    if (_cachedLength != dataLength)
                        throw new FileFormatException("CompressLengthMismatch");
