...
using esDigitalSignature.iTextSharp.text.io;
...
namespace esDigitalSignature.iTextSharp.text.pdf {
    ...
    public class PdfSignatureAppearance {
        ...
        */
        ...
        /**
         * Sets the signature to be visible. It creates a new visible signature field in default position.
         * @param page the page to place the field. The fist page is 1
         * @param fieldName the field name or <CODE>null</CODE> to generate automatically a new field name
         */
        virtual public void SetVisibleSignature(int page, String fieldName)
        {
            if (fieldName != null)
            {
                if (fieldName.IndexOf('.') >= 0)
            ...
            //TOANTK Vị trí Signature field
            AcroFields af1 = writer.GetAcroFields();
            Rectangle pageSize = writer.reader.GetPageSizeWithRotation(page);
            float urx = pageSize.Width - 10 - af1.Fields.Count * 210;
            float ury = pageSize.Height -10;
            float llx = urx - 200;
            float lly = ury - 70;
            Rectangle pageRect = new esDigitalSignature.iTextSharp.text.Rectangle(llx, lly, urx, ury);
            this.pageRect = new Rectangle(pageRect);
            this.pageRect.Normalize();
            rect = new Rectangle(this.pageRect.Width, this.pageRect.Height);
        ...
        virtual public PdfTemplate GetAppearance() {
            ...
            if (app[2] == null) {
                ...
                if (layer2Text == null) {
                    ...
                    if (x500name != null) {
                        name = ConvertToUnSign(x500name.GetField("CN"));
                        if (name == null)
                            name = ConvertToUnSign(x500name.GetField("E"));
                    }
                    if (name == null)
                        name = "";
                    ...
                    if (reason != null)
                        buf.Append('\n').Append(reasonCaption).Append(ConvertToUnSign(reason));
                    if (location != null)
                        buf.Append('\n').Append(locationCaption).Append(ConvertToUnSign(location));
                    text = buf.ToString();
                }
                else
                    text = layer2Text;
                ...
                Font font;
                if (layer2Font == null)
                    font = new Font(Font.FontFamily.ARIAL);
                else
        ...
        */    
        ...
        //TOANTK: chuyển có dấu thành không dấu
