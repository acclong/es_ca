...
namespace SignPackage
{
    internal static class PackagingUtilities
    {
        ...
        {
            if (!s.CanRead)
                throw new NotSupportedException("ReadNotSupported");
            ...
            if (offset < 0)
            {
                throw new ArgumentOutOfRangeException("offset", "OffsetNegative");
            }

            if (count < 0)
            {
                throw new ArgumentOutOfRangeException("count", "ReadCountNegative");
            }
            ...
            checked     // catch any integer overflows
            {
                ...
                {
                    throw new ArgumentException("ReadBufferTooSmall", "buffer");
                }
            }
        }
        ...
        {
            if (!s.CanWrite)
                throw new NotSupportedException("WriteNotSupported");
            ...
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException("count", "WriteCountNegative");
            }

            checked
            {
                if (offset + count > buffer.Length)
                    throw new ArgumentException("WriteBufferTooSmall", "buffer");
            }
        }
