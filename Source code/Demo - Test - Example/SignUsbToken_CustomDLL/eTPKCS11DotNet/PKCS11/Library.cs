...
namespace eToken
{
    public static partial class PKCS11
    {
        public class Exception : System.Exception
        {
            ...
            {
                if (rv != 0) throw new Exception(rv, "eToken.PKCS11+Exception: error " + rv.ToString() + (ErrorName.ContainsKey(rv)? " '" + ErrorName[rv] + "'." : "."));
            }

            #region TOANTK
            internal Exception(int error, string mess) : base(mess)
            {
