...
namespace ES.CA_WebService
{
    ...
    public class CAService : System.Web.Services.WebService
    {
        ...
        {
            //CSDL
            DAL_SqlConnector.ConnectionString = GetDatabaseString(0);
        ...
        [WebMethod(Description = "Thông tin xác thực bảng kê của đơn vị trong nhiều ngày")]
        public DataTable TT_XacThucThongKe_TuNgayDenNgay_EPTC(string Ma_NM, DateTime TuNgay, DateTime DenNgay)
        {
            DAL_SqlConnector.ConnectionString = GetDatabaseString(1);

            BUSThanhToan bus = new BUSThanhToan();

            return bus.TT_XacThucThongKe_TuNgayDenNgay_EPTC(Ma_NM, TuNgay, DenNgay);
        }

        [WebMethod(Description = "Thông tin chi tiết xác thực bảng kê của đơn vị trong 1 ngày")]
        public DataTable TT_XacThucThongKe_Ngay_EPTC(string Ma_NM, DateTime Ngay)
        {
            ...
            return bus.TT_XacThucThongKe_Ngay_EPTC(Ma_NM, Ngay);
        }
        
        [WebMethod(Description = "Cập nhật thông tin xác nhận bảng kê của đơn vị trong 1 ngày")]
        public bool InsertXacThucBangKe_EPTC(string ma_NM, DateTime ngay, int lanXacThuc, bool XacThuc, string LyDo, string NguoiXacThuc, DateTime NgayXacThuc)
        {
            ...
            return bus.InsertXacThucBangKe_EPTC(ma_NM, ngay, lanXacThuc, XacThuc, LyDo, NguoiXacThuc, NgayXacThuc);
        }

        [WebMethod(Description = "Lấy thông tin connectstring, 0 là CA, 1 là TTTT")]
        private string GetDatabaseString(int index)
        {
            string databaseTTTT = ConfigurationManager.ConnectionStrings["databaseTTTT"].ConnectionString;
            string database = ConfigurationManager.ConnectionStrings["database"].ConnectionString;
