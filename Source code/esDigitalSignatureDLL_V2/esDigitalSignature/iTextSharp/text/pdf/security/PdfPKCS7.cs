...
namespace esDigitalSignature.iTextSharp.text.pdf.security {
    ...
    internal class PdfPKCS7 {
        ...
        public PdfPKCS7(ICipherParameters privKey, ICollection<X509Certificate> certChain, 
            ...
            //
            //Edited by Toantk on 11/5/2015
            //Add check Count in case certChain=null when GetHashValue()
            if (certs.Count > 0)
                signCert = certs[0];
        ...
        //Toantk
        virtual public byte[] getContentAttributeBytes(byte[] secondDigest, byte[] ocsp, ICollection<byte[]> crlBytes, CryptoStandard sigtype)
        {
            return GetContentAttributeSet(secondDigest, ocsp, crlBytes, sigtype).GetEncoded(Asn1Encodable.Der);
        }

        //Toantk
        private DerSet GetContentAttributeSet(byte[] secondDigest, byte[] ocsp, ICollection<byte[]> crlBytes, CryptoStandard sigtype)
        {
            Asn1EncodableVector attribute = new Asn1EncodableVector();
            Asn1EncodableVector v = new Asn1EncodableVector();
            v.Add(new DerObjectIdentifier(SecurityIDs.ID_CONTENT_TYPE));
