...
namespace esDigitalSignature.iTextSharp.text.pdf.security {
     ...
     * @author Paulo Soares
     */
    internal static class MakeSignature {
        /** The Logger instance. */
         ...
         * @throws Exception 
         */
        public static void SignDetached(PdfSignatureAppearance sap, IExternalSignature externalSignature, ICollection<X509Certificate> chain, 
            ICollection<ICrlClient> crlList, IOcspClient ocspClient, ITSAClient tsaClient, int estimatedSize, CryptoStandard sigtype, 
            HSMServiceProvider providerHSM)
        {
            ...
            byte[] sh = sgn.getAuthenticatedAttributeBytes(hash, cal, ocsp, crlBytes, sigtype);
            byte[] extSignature = externalSignature.Sign(sh, providerHSM);
            sgn.SetExternalDigest(extSignature, null, externalSignature.GetEncryptionAlgorithm());
