...
using System.IO;
using System.Text;
using esDigitalSignature.iTextSharp.awt.geom;
using esDigitalSignature.iTextSharp.text.exceptions;
using esDigitalSignature.iTextSharp.text.pdf.interfaces;
using esDigitalSignature.iTextSharp.text.pdf.intern;
using esDigitalSignature.iTextSharp.text.error_messages;
        ...
        */
        ...
        virtual public void Rectangle(Rectangle rectangle) {
            ...
            if (rectangle.UseVariableBorders) {
                ...
                if (rectangle.BorderWidth != esDigitalSignature.iTextSharp.text.Rectangle.UNDEFINED) {
                ...
                if (rectangle.HasBorder(esDigitalSignature.iTextSharp.text.Rectangle.BOX)) {
                    this.Rectangle(x1, y1, x2 - x1, y2 - y1);
                }
                    // if the border isn't a rectangle, the different sides are added apart
                else {
                    if (rectangle.HasBorder(esDigitalSignature.iTextSharp.text.Rectangle.RIGHT_BORDER)) {
                        MoveTo(x2, y1);
                        LineTo(x2, y2);
                    }
                    if (rectangle.HasBorder(esDigitalSignature.iTextSharp.text.Rectangle.LEFT_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x1, y2);
                    }
                    if (rectangle.HasBorder(esDigitalSignature.iTextSharp.text.Rectangle.BOTTOM_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x2, y1);
                    }
                    if (rectangle.HasBorder(esDigitalSignature.iTextSharp.text.Rectangle.TOP_BORDER)) {
                        MoveTo(x1, y2);
                        LineTo(x2, y2);
                    }
                }
