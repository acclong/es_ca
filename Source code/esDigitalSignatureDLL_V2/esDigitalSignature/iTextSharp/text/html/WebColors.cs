...
/*
 * $Id: WebColors.cs 787 2014-07-17 09:05:54Z asubach $
 * 
 *
...
namespace esDigitalSignature.iTextSharp.text.html {
    ...
    internal class WebColors : Dictionary<string, int[]> {
        ...
        private static bool MissingHashColorFormat(String colStr) {
            ...
            if (len == 3 || len == 6) {
                ...
                Regex match = new Regex("[0-9a-f]{" + len + "}");
                return match.IsMatch(colStr);
            }
            return false;
        }
