...
namespace esDigitalSignature
{
    ...
    public class Common
    {
        ...
        /// <summary>
        /// Địa chỉ timestamp server
        /// </summary>
        public static string TSAServerUri = "http://tsa.vnpt-ca.vn/";//"http://tsa.belgium.be/connect";
            ...
            out X509ChainStatus certificateStatus)
        {
            ...
            chain.ChainPolicy.VerificationFlags = X509VerificationFlags.IgnoreCertificateAuthorityRevocationUnknown | X509VerificationFlags.IgnoreRootRevocationUnknown;
        ...
        //Toantk 24/8/2016: Test Bouncy Castle CRL and OCSP
        private static byte[] GetEncoded(X509Certificate2 certificate)
        {
            try
            {
                X509Chain x509Ch = new X509Chain();
                x509Ch.Build(certificate);
                //Certificate
                Org.BouncyCastle.X509.X509CertificateParser cp = new Org.BouncyCastle.X509.X509CertificateParser();
                Org.BouncyCastle.X509.X509Certificate cert = cp.ReadCertificate(certificate.RawData);
                Org.BouncyCastle.X509.X509Certificate issuerCert = cp.ReadCertificate(x509Ch.ChainElements[1].Certificate.RawData);

                esDigitalSignature.iTextSharp.text.pdf.security.OcspClientBouncyCastle ocspClient = new esDigitalSignature.iTextSharp.text.pdf.security.OcspClientBouncyCastle();
                Org.BouncyCastle.Ocsp.BasicOcspResp basicResponse = ocspClient.GetBasicOCSPResp(cert, issuerCert, null);
                if (basicResponse != null)
                {
                    Org.BouncyCastle.Ocsp.SingleResp[] responses = basicResponse.Responses;
                    if (responses.Length == 1)
                    {
                        Org.BouncyCastle.Ocsp.SingleResp resp = responses[0];
