...
namespace esDigitalSignature.eToken
{
    internal static partial class PKCS11
    {
        public class Exception : System.Exception
        {
            ...
            {
                if (rv != 0) throw new Exception(rv, "PKCS#11 error " + rv.ToString() + (ErrorName.ContainsKey(rv) ? ": '" + ErrorName[rv] + "'." : "."));
            }

            #region TOANTK
            internal Exception(int error, string mess)
                : base(mess)
            {
                this.error = error; //Exception of type 'eToken.PKCS11+Exception' was thrown.
            }
            #endregion
        }
        ...
        */
        ...
        /*@@C_WrapKey
          ...
          * @param session [in] the session's handle
          * @param mech [in] key generation mech
          * @param wrappingKey [in] wrapping key's handle
          * @param key [in] key's handle
          * @param wrappedKey [out] gets wrapped key data
          * @param wrappedKeySize [out] gets size of wrapped key
          * @return zero if successful CK_RV value > zero in case of failure
          */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        internal delegate int type_C_WrapKey(int session, IntPtr mech, int wrappingKey, int key, IntPtr wrappedKey, out int wrappedKeySize);

        /*@@C_UnwrapKey
          * Function C_UnwrapKey() unwraps (decrypts) a wrapped key, creating a new key object.
          * @param session [in] the session's handle
          * @param mech [in] key generation mech
          * @param unwrappingKey [in] unwrapping key's handle
          * @param wrappedKey [in] wrapped key
          * @param wrappedKeySize [in] wrapped key length
          * @param t [in] template for new key
          * @param tSize [in] # of attrs in template
          * @param key [out] gets handle of new key
          * @return zero if successful CK_RV value > zero in case of failure
          */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        internal delegate int type_C_UnwrapKey(int session, IntPtr mech, int unwrappingKey, IntPtr wrappedKey, int wrappedKeySize, IntPtr t, int tSize, out int key);
        ...
        /*@@C_DeriveKey
          ...
          * @param session [in] the session's handle
          * @param mech [in] key generation mech
          * @param baseKey [in] base key's handle
          * @param tPub [in] template for new key
          * @param tPubSize [in] # of attrs in template
          * @param key [out] gets handle of new key
          * @return zero if successful CK_RV value > zero in case of failure
          */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        internal delegate int type_C_DeriveKey(int session, IntPtr mech, int baseKey, IntPtr tPub, int tPubSize, out int key);
        /********* Create by TOANTK *********/
        ...
        internal class CK_FUNCTION_LIST
        {
            ...
            /** Function C_WrapKey() wraps (i.e., encrypts) a key. */
            [MarshalAs(UnmanagedType.FunctionPtr)]
            public type_C_WrapKey C_WrapKey;
            /** Function C_UnwrapKey() unwraps (decrypts) a wrapped key, creating a new key object. */
            [MarshalAs(UnmanagedType.FunctionPtr)]
            public type_C_UnwrapKey C_UnwrapKey;
            //27/3/2015
            /** Function C_DeriveKey() derives a key from a base key, creating a new key object. */
            [MarshalAs(UnmanagedType.FunctionPtr)]
            public type_C_DeriveKey C_DeriveKey;
            /********* Create by TOANTK *********/
            /** Function C_SeedRandom() mixes additional seed material into the token's random number generator. */
            [MarshalAs(UnmanagedType.FunctionPtr)]
