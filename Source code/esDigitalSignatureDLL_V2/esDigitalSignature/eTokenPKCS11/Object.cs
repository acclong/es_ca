...
namespace esDigitalSignature.eToken
{
  internal static partial class PKCS11
  {
    ...
    public partial struct Object
    {
      ...
      // 27/3/2015
      public Object DeriveKey(Session session, Mechanism mechanism, Attribute[] t)
      {
          int objid;
          Buffer tBuf = template_to_buffer(t, false);
          Buffer mech = mechanism_to_buffer(mechanism);
          int rv = PKCS11.fl.C_DeriveKey(session.id, mech.ptr, id, tBuf.ptr, t.Length, out objid);
          Exception.check(rv);
          return new Object(objid);
      }
      /*********Create by TOANTK *********/
