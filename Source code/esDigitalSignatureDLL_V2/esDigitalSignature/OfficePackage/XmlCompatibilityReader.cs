    ...
    internal sealed class XmlCompatibilityReader : XmlWrappingReader
    {
        ...
        private bool ReadStartElement(ref bool more)
        {
            ...
            if (object.ReferenceEquals(namespaceName, CompatibilityUri))
            {
                ...
                {
                    Error("XCRUnknownCompatElement", elementName);
                }
            ...
            else
            {
                ...
                else
                {
                    if (Scope.InAlternateContent)
                    {
                        ...
                        Error("XCRInvalidACChild", Reader.Name);
                    }
                ...
                if (isSupported)
                {
                    ...
                    else
                    {
                        ...
                        if (!_namespaceMap.TryGetValue(mappedNamespace, out tempNamespace))
                        {
                            ...
                            {
                                Error("XCRCompatCycle", mappedNamespace);
                            }
            ...
            foreach (string pair in content.Trim().Split(' '))
            {
                ...
                if (!String.IsNullOrEmpty(pair))
                {
                    ...
                    if (colonIndex <= 0 || colonIndex >= length - 1 || colonIndex != pair.LastIndexOf(':'))
                    {
                        ...
                        Error("XCRInvalidFormat", callerContext);
                    }
                ...
                do
                {
                    ...
                    if (ShouldIgnoreNamespace(namespaceName))
                    {
                        ...
                        if (object.ReferenceEquals(namespaceName, CompatibilityUri))
                        {
                            ...
                            {
                                Error("XCRUnknownCompatAttrib", attributeName);
                            }
        ...
        private void HandleChoice(int elementDepth, ref bool more)
        {
            ...
            if (AttributeCount != 1)
            {
                ...
                Error("XCRInvalidAttribInElement", attributeName, Choice);
            }
        ...
        private void HandleFallback(int elementDepth, ref bool more)
        {
            ...
            if (Scope.FallbackSeen)
            {
                ...
                Error("XCRMultipleFallbackFound");
            }
            ...
            if (AttributeCount != 0)
            {
                ...
                Error("XCRInvalidAttribInElement", attributeName, Fallback);
            }
        ...
        private void HandleMustUnderstand(int elementDepth)
        {
            foreach (string namespaceUri in PrefixesToNamespaces(Reader.Value))
            {
                if (!IsNamespaceKnown(namespaceUri))
                {
                    Error("XCRMustUnderstandFailed", namespaceUri);
                }
            }
        }
            ...
            public void Verify()
            {
                ...
                if (_processContents != null)
                {
                    foreach (string key in _processContents.Keys)
                    {
                        ...
                        {
                            _reader.Error("XCRNSProcessContentNotIgnorable", key);
                        }
                    }
                }
                ...
                if (_preserveElements != null)
                {
                    foreach (string key in _preserveElements.Keys)
                    {
                        if (!IsIgnorableAtCurrentScope(key))
                        {
                            _reader.Error("XCRNSPreserveNotIgnorable", key);
                        }
                    }
                }
        ...
        class ProcessContentSet
        {
            ...
            public void Add(string elementName)
            {
                if (ShouldProcessContent(elementName))
                {
                    if (elementName == "*")
                    {
                        _reader.Error("XCRDuplicateWildcardProcessContent", _namespaceName);
                    }
                    else
                    {
                        _reader.Error("XCRDuplicateProcessContent", _namespaceName, elementName);
                    }
                }
                ...
                if (elementName == "*")
                {
                    if (_names != null)
                    {
                        _reader.Error("XCRInvalidProcessContent", _namespaceName);
                    }
                    else
                    {
                        _all = true;
        ...
        class PreserveItemSet
        {
            ...
            public void Add(string itemName)
            {
                if (ShouldPreserveItem(itemName))
                {
                    if (itemName == "*")
                    {
                        _reader.Error("XCRDuplicateWildcardPreserve", _namespaceName);
                    }
                    else
                    {
                        _reader.Error("XCRDuplicatePreserve", itemName, _namespaceName);
                    }
                }
                ...
                if (itemName == "*")
                {
                    if (_names != null)
                    {
                        _reader.Error("XCRInvalidPreserve", _namespaceName);
                    }
                    else
                    {
                        _all = true;
