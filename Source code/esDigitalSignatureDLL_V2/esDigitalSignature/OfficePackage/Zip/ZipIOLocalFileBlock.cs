...
///////////////////////////////////////////////////////////////////////                    
        ...
        internal Stream GetStream(FileMode mode, FileAccess access)
        {
            ...
            switch(mode)
            {
                ...
                case FileMode.CreateNew:       
                    ...
                    throw new ArgumentException("FileModeUnsupported: CreateNew");
                case FileMode.Append:
                    throw new ArgumentException("FileModeUnsupported: Append");
                case FileMode.Truncate:
                    throw new ArgumentException("FileModeUnsupported: Truncate");
                default:
        ...
        internal void CheckDisposed()
        {
            if (_disposedFlag)
            {
                throw new ObjectDisposedException(null, "ZipFileItemDisposed");            
            }
        }
                                            ...
                                            ZipIOCentralDirectoryFileHeader centralDirFileHeader)
        {
            ...
            {
                throw new NotSupportedException("ZipNotSupportedCompressionMethod");
            }
        ...
        static private void CheckFileAccessParameter(Stream stream, FileAccess access)
        {
            switch(access)
            {
                case FileAccess.Read:
                    if (!stream.CanRead)
                    {
                        throw new ArgumentException ("CanNotReadInWriteOnlyMode");
                    }
                    break;
                case FileAccess.Write:
                    if (!stream.CanWrite)
                    {
                        throw new ArgumentException ("CanNotWriteInReadOnlyMode");
                    }
                    break;                
                case FileAccess.ReadWrite:
                    if (!stream.CanRead || !stream.CanWrite)
                    {
                        throw new ArgumentException ("CanNotReadWriteInReadOnlyWriteOnlyMode");
                    }
                    break;                
                default:
