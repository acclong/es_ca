...
namespace esDigitalSignature.OfficePackage.Cryptography.Xml
{
    ...
    using esDigitalSignature.OfficePackage.Cryptography;
    ...
    internal class SignedXml {
        ...
        #region TOANTK
        public void ComputeSignature(HSMServiceProvider providerHSM) {
            SignedXmlDebugLog.LogBeginSignatureComputation(this, m_context);
            ...
            if (SignedInfo.SignatureMethod == null)
            {
                if (key is esDigitalSignature.HSM_DSA)
                {
                    SignedInfo.SignatureMethod = XmlDsigDSAUrl;
                }
                else if (key is esDigitalSignature.HSM_RSA)
                {
                    // Default to RSA-SHA1
                    if (SignedInfo.SignatureMethod == null)
                        SignedInfo.SignatureMethod = XmlDsigRSASHA1Url;
            ...
            m_signature.SignatureValue = providerHSM.SignHash(hashAlg.Hash, sOID);
        }
