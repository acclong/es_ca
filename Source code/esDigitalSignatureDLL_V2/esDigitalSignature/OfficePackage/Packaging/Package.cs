...
namespace esDigitalSignature.OfficePackage
{
    ...
    /// </summary>
    internal abstract class Package : IDisposable
    {
                            ...
                            CompressionOption compressionOption)
        {
            ...
            if (_partList.ContainsKey(validatedPartUri))
                throw new InvalidOperationException("PartAlreadyExists");
        ...
        public PackagePart GetPart(Uri partUri)
        {
            ...
            if (returnedPart == null)
                throw new InvalidOperationException("PartDoesNotExist");
            else
                return returnedPart;
        }
            ...
            if (_partCollection == null)
            {
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    if (seenPartUris.ContainsKey(partUri))
                        throw new FileFormatException("BadPackageFormat");
                    else
                    {
        ...
        public PackageRelationship GetRelationship(string id)
        {
            ...
            if (returnedRelationship == null)
                throw new InvalidOperationException("PackageRelationshipDoesNotExist");
            else
        ...
        internal void ThrowIfInStreamingCreation(string methodName)
        {
            if (_inStreamingCreation)
                throw new IOException("OperationIsNotSupportedInStreamingProduction: " + methodName);
        }
        ...
        {
            if (!InStreamingCreation)
                throw new IOException("MethodAvailableOnlyInStreamingCreation: " + methodName);
        }
        ...
        internal void ThrowIfReadOnly()
        {
            if (_openFileAccess == FileAccess.Read)
                throw new IOException("CannotModifyReadOnlyContainer");
        }
        ...
        internal void ThrowIfWriteOnly()
        {
            if (_openFileAccess == FileAccess.Write)
                throw new IOException("CannotRetrievePartsOfWriteOnlyContainer");
        }
        ...
        private void AddIfNoPrefixCollisionDetected(PackUriHelper.ValidatedPartUri partUri, PackagePart part)
        {
                ...
                && followingPartName[normalizedPartName.Length] == PackUriHelper.ForwardSlashChar))
            {
                ...
                throw new InvalidOperationException("PartNamePrefixExists");
            }
        }
            ...
            if (streaming)
            {
                if (packageMode == FileMode.Create || packageMode == FileMode.CreateNew)
                {
                    if (packageAccess != FileAccess.Write)
                        throw new IOException("StreamingPackageProductionImpliesWriteOnlyAccess");
                    if (   packageShare != null
                        && packageShare != FileShare.Read && packageShare != FileShare.None)
                        throw new IOException("StreamingPackageProductionRequiresSingleWriter");
                }
                else
                {
                    // Blanket exception pending design of streaming consumption.
                    throw new NotSupportedException("StreamingModeNotSupportedForConsumption");
                }
            }
        }
        ...
        private static Stream ValidateModeAndAccess(Stream s, FileMode mode, FileAccess access)
        {
            ...
            if (!s.CanWrite && (access == FileAccess.ReadWrite || access == FileAccess.Write))
                throw new IOException("IncompatibleModeOrAccess");
        ...
        {
            if (_disposed == true)
                throw new ObjectDisposedException(null, "ObjectDisposed");
        }
        ...
        private sealed class RestrictedStream : Stream
        {
            ...
            public override void SetLength(long newLength)
            {
                ...
                if (_canWrite)
                    ...
                    throw new NotSupportedException("ReadOnlyStream");
            }
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                if (_canRead)
                    ...
                    throw new NotSupportedException("WriteOnlyStream");
            }
