...
namespace esDigitalSignature.OfficePackage
{
    ...
    internal class XmlDigitalSignatureProcessor
    {
            ...
            IEnumerable<Reference> objectReferences)
        {
            ...
            try
            {
                ...
                using (HashAlgorithm hashAlgorithm = GetHashAlgorithm(_hashAlgorithmName))
                {
                    ...
                    _signedXml.AddObject(GenerateObjectTag(hashAlgorithm, parts, relationshipSelectors, signingTime, signatureId));
                    ...
                    //Toantk 10/8/2015: Thêm QualifyingProperties Object để tương thích XAdES-BES
                    _signedXml.AddObject(GenerateQualifyingPropertiesTag(signingTime, signatureId));
                }
                ...
                //Toantk 30/8/2015: add reference from SignedInfo to Office object tag (for compatibility with MS Office 2007)
                ...
                //Toantk 10/8/2016: add the XAdES-BES reference
                var xadesRef = new Reference(XTable.Get(XTable.ID.XadesSignedPropertiesIdLink));
                xadesRef.DigestMethod = _hashAlgorithmName;
                xadesRef.Type = XTable.Get(XTable.ID.XadesSignatureNamespaceRoot) + "SignedProperties";
                xadesRef.AddTransform(new esDigitalSignature.OfficePackage.Cryptography.Xml.XmlDsigC14NTransform());
                _signedXml.AddReference(xadesRef);
                ...
                String signatureId)
        {
            ...
            xDoc.DocumentElement.AppendChild(XmlSignatureProperties.AssembleSignatureProperties(xDoc, signingTime, _manager.TimeFormat, signatureId));
        ...
        //Toantk 10/8/2016
        private DataObject GenerateQualifyingPropertiesTag(DateTime signingTime, String signatureId)
        {
            XmlDocument xDoc = new XmlDocument();
            xDoc.AppendChild(xDoc.CreateNode(XmlNodeType.Element, "root", "namespace")); // dummy root
            xDoc.DocumentElement.AppendChild(XmlSignatureProperties.AssembleSignatureQualifyingProperties(xDoc, _manager.HashAlgorithm, signatureId, signingTime, _manager.TimeFormat, _certificate));
        ...
        /// <summary>
        /// Toantk 15/8/2016: Add a timestamp to the signature, complaint with the XAdES-T 1.3.1 standard.
        /// </summary>
        /// <remarks>
        ...
        private void ExtendToXadesT(ref XmlElement signature)
        {
            TSA.ITimestampProvider timestampProvider = new TSA.Rfc3161TimestampProvider(new Uri(Common.TSAServerUri));

            if (signature == null)
            {
                throw new ArgumentNullException("signature", "A signature is required for XAdES-T");
            }
            ...
            namespaceManager.AddNamespace("xd", "http://uri.etsi.org/01903/v1.3.2#");
            XmlNode sigValue = signature.SelectSingleNode("./ds:SignatureValue", namespaceManager);
            ...
            string hashValueString = GenerateDigestValue(stream, SignedXml.XmlDsigC14NTransformUrl, GetHashAlgorithm(_hashAlgorithmName));
            byte[] hashedSignatureValue = System.Convert.FromBase64String(hashValueString);
            //Get the timestamp.
            byte[] timestampHash = timestampProvider.GetTimestampFromDocumentHash(hashedSignatureValue, _hashAlgorithmName);

            //Generate XML timestamp
            XmlDocument xDoc = new XmlDocument();
            xDoc.AppendChild(xDoc.CreateNode(XmlNodeType.Element, "root", "namespace")); // dummy root
            XmlElement timestampValue = XmlSignatureProperties.AssembleSignatureTimeStamp(xDoc, SignedXml.XmlDsigC14NTransformUrl, timestampHash != null ? Convert.ToBase64String(timestampHash) : null);

            //Add timestamp to the signature
            XmlElement unsignedSigProps = (XmlElement)signature.SelectSingleNode("./ds:Object/xd:QualifyingProperties/xd:UnsignedProperties/xd:UnsignedSignatureProperties", namespaceManager);
            var imported = unsignedSigProps.OwnerDocument.ImportNode(timestampValue, true);
            ...
            IEnumerable<Reference> objectReferences)
        {
            ...
            try
            {
                ...
                //Toantk 30/8/2015: add reference from SignedInfo to Office object tag (for compatibility with Office 2007)
                Reference objectReference1 = new Reference(XTable.Get(XTable.ID.OpcOfficeLinkAttrValue));
                objectReference1.Type = XTable.Get(XTable.ID.W3CSignatureNamespaceRoot) + "Object";
                objectReference1.DigestMethod = _hashAlgorithmName;
                _signedXml.AddReference(objectReference1);
